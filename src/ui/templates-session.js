...
export function controller(state) {
  ...
  return `<main class="controller-shell"><header class="controller-header glass">${playerAvatar(player, 'avatar--large')}<span><small>${state.mode === 'online' ? `ONLINE · ${escapeHtml(state.online.roomCode ?? '')}` : 'CONTROLLER'}</small><strong>${escapeHtml(player.name)}</strong><em>${escapeHtml(player.ticker)}</em></span>${state.mode === 'online' ? '<span class="feed-label">SYNCED</span>' : '<button class="button" data-action="switch-player">SWITCH</button>'}</header>${!round ? `<section class="controller-card glass"><h1>Waiting for a session</h1><p>${state.mode === 'online' ? 'The host has not opened the online market yet.' : 'Return to the host screen and create a local room.'}</p></section>` : `<section class="controller-card glass"><span class="eyebrow">ROUND ${round.index + 1} · ${state.session.phase}</span><h1>${escapeHtml(getGame(round.gameId).name)}</h1><p>${escapeHtml(getGame(round.gameId).instructions)}</p>${[PHASES.LOCKED, PHASES.GAME].includes(round.phase) ? `<button class="button button--warm button--large full-width" data-action="${round.phase === PHASES.GAME ? 'resume-game' : 'begin-game'}">${round.phase === PHASES.GAME ? 'PLAY / RESUME' : state.mode === 'online' ? 'WAIT FOR HOST' : 'PLAY ROUND'}</button>` : ''}</section><section class="controller-card glass"><div class="card-topline"><span class="eyebrow">FRIEND WALLET</span><b>${money(friendSnapshot.equity)}</b></div><div class="controller-markets">${Object.values(state.markets.friend).map((asset) => `<button data-action="trade-asset" data-market="friend" data-symbol="${asset.symbol}"><span><strong>${asset.symbol}</strong><small>${money(asset.price)}</small></span><em class="${asset.roundChange >= 0 ? 'positive' : 'negative'}">${signedPercent(asset.roundChange)}</em></button>`).join('')}</div></section>`}</main>`;
}
